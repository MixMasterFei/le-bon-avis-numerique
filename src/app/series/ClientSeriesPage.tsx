...
export function ClientSeriesPage({ initialData, initialFilters, initialPage }: ClientSeriesPageProps) {
  ...
  // Fetch series from API (client-side, for filter/page changes)
  useEffect(() => {
      ...
      try {
        ...
        if (dbRes.ok) {
          ...
          if (dbData.series && dbData.series.length > 0) {
            const mapped = dbData.series.map((s: ApiMediaRecord) => mapApiToMediaItem(s))
            if (!cancelled) {
              setDbSeries(mapped)
  ...
  const totalCount = dbTotalResults ?? dbSeries.length
