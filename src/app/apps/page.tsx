...
export default function AppsPage() {
  ...
  const apps = useMemo(() => {
    let items = mockMediaItems.filter((m) => m.type === "APP")
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Trouvez les meilleures applications éducatives et de divertissement pour vos enfants.
        </p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-6">
            <p className="text-gray-600">
              {apps.length} application{apps.length !== 1 ? "s" : ""} trouvée{apps.length !== 1 ? "s" : ""}
            </p>
          </div>
          ...
          {apps.length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
              {apps.map((app) => (
                <MediaCard key={app.id} media={app} />
              ))}
            </div>
          ) : (
            <div className="text-center py-16 text-gray-500">
              <Smartphone className="h-12 w-12 mx-auto mb-4 opacity-50" />
              <p className="text-lg font-medium">Aucune application trouvée</p>
