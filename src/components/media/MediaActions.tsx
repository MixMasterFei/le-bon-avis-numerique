...
function PillButton({
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold transition-all disabled:opacity-60 hover:-translate-y-0.5"
      style={{
        background: p.card,
        color: active ? activeColor || p.ink : p.ink,
        border: `1px solid ${active ? activeColor || p.line2 : p.line2}`,
        boxShadow: "0 2px 6px rgba(30,26,21,0.05)",
      }}
    >
      {children}
    </button>
