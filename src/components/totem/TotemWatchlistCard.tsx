...
export interface TotemWatchlistCardProps {
  toolCallId: string
  mediaId: string
  mediaTitle: string
  resolved?: { accepted: boolean; alreadyPresent?: boolean; reason?: string }
  onAccept: (toolCallId: string, mediaId: string) => Promise<void> | void
  onDecline: (toolCallId: string) => void
}

export function TotemWatchlistCard({
  toolCallId,
  mediaId,
  mediaTitle,
  resolved,
  ...
  const handleAccept = async () => {
    ...
    setBusy(true)
    await onAccept(toolCallId, mediaId)
    setBusy(false)
  }
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2">
        <Bookmark className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-amber-700" />
        <div className="min-w-0 flex-1">
          <div className="text-xs italic text-amber-900/80">
            Ajouter à votre liste <em>à voir plus tard</em> ?
          </div>
      ...
      {isResolved ? (
        <div className="mt-2 text-xs text-amber-900/80">
          {resolved!.accepted && !resolved!.alreadyPresent && "✓ Ajouté à votre liste"}
          {resolved!.accepted && resolved!.alreadyPresent && "C'est déjà dans votre liste — bonne mémoire."}
