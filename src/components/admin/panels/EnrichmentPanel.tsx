...
function initialTypeFromQuery(): MediaType {
  ...
  return parseMediaType(new URLSearchParams(window.location.search).get("type"))
}

export function EnrichmentPanel({ initialType }: { initialType?: string }) {
  const [stats, setStats] = useState<EnrichmentStats | null>(null)
  const [loading, setLoading] = useState(true)
  const [enriching, setEnriching] = useState(false)
  const [selectedType, setSelectedType] = useState<MediaType>(
    parseMediaType(initialType) !== "all" ? parseMediaType(initialType) : initialTypeFromQuery(),
  )
  ...
  useEffect(() => {
    const t = parseMediaType(initialType)
    if (t !== "all") setSelectedType(t)
    else {
      const fromQuery = initialTypeFromQuery()
      if (fromQuery !== "all") setSelectedType(fromQuery)
    }
  }, [initialType])
