 ...
 * MOVIE, TV, and GAME types.
 *
 * Shared by the `/api/db/expert-picks` route (client rails with the reload
 * button) and the homepage hero, which calls it directly from the server so
 * the poster showcase ships in the initial HTML instead of a post-hydration
 * fetch (the desktop LCP element lives there).
 */
...
export interface ExpertPicksOptions {
  /** Number of items to return (default 6). */
  limit?: number
  /** Shuffle seed (default: weekly seed). */
  seed?: number
  /** Age cap, clamped to [0, 18] (default 12). */
  maxAge?: number
}

export async function getExpertPicks({
  limit = 6,
  seed = getWeekSeed(),
  maxAge,
}: ExpertPicksOptions = {}): Promise<ExpertPickItem[]> {
  ...
  const ageCap = Number.isFinite(maxAge)
    ? Math.min(18, Math.max(0, maxAge as number))
    : 12
