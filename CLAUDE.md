...
TOTEM_PUBLIC                      # Set to "true" (or "auth") to open the Totem Assistant chatbot to all AUTHENTICATED users. Totem is account-gated by design — no flag value grants anonymous access (family context is what makes answers reliable; anon rate limiting is per-instance). Default: admin-only alpha (dock + /api/totem/chat gated by ADMIN role). Gate lives in src/lib/totem/access.ts.
