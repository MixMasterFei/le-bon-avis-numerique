...
| Composed board sections | `src/app/decouverte/blocks/` — `HeroMatch` (full-bleed backdrop for MOVIE/TV, poster-led for GAME: **`backdropUrl` is always null for games**), `BoardSections` (grid/rail/editorial), `EditorialSources` (blog/upcoming — the news block was removed from the registry: it read as filler on a selection board), `DeferredBlock` (the sections that reach TMDB or Sanity, streamed in their own Suspense boundary so a slow third party never delays the answer). |
